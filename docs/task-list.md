...
| Phase | テーマ | 状態 |
|---|---|---|
| **0** | 基盤導入（zip 展開・AGENTS/.agent/docs 移植・疎通） | 本タスクで実施 |
| **1** | 地図の堅牢化（bbox/クラスタ/SSR分離/a11y/パフォーマンス） | 未着手 |
| **2** | API / DB 強化（facets キャッシュ・近隣・seed 冪等・index） | 未着手 |
| **3** | スクレイパー強化（リトライ・差分・重複排除・ポライトネス） | 未着手 |
| **4** | UI 磨き（M3 トークン整理・motion 予算・フィルタ永続化・免責） | 未着手 |
| **5** | 運用 / 品質（テスト・E2E・監視・workflow 可観測性） | 未着手 |
...
| SETUP-5 | 疎通確認（typecheck / lint / build） | 実装中 | 80% | SETUP-0〜4 | `npm run typecheck` / `npm run lint` / `npm run build` が pass（DB 無しでも GeoJSON 経路でビルド） | `npm run ...` ログ |
