...
npm run typecheck             # 型が通ること（プロジェクト初期化前は未整備でも可、その場合は npm ci 成功まで確認）
npm run build 2>&1 | tail -20 # Next.js がビルドできること（DB 無しでも GeoJSON 経路でビルド可能）
```
→ 問題なければ作業再開。
...
- `data/spots.geojson`（大容量）は `git reset --hard` 後に正しい件数（752件規模）に戻っていることを `wc -c` / `jq .features | length` 的な確認で担保する。
